...
class MinHeap {
    ...
    private int parent(int keyIndex) {
        return (keyIndex - 1) / 2;
    }

    // Get the Left Child index for the given index
    private int left(int keyIndex) {
        return 2 * keyIndex + 1;
    }
    ...
    private void MinHeapify(int parentIndex) {
        ...
        if (leftIndex < current_heap_size && heapArray[leftIndex] < heapArray[smallestIndex]) {
            smallestIndex = leftIndex;
        }
        if (rightIndex < current_heap_size && heapArray[rightIndex] < heapArray[smallestIndex]) {
            smallestIndex = rightIndex;
        }

        if (smallestIndex != parentIndex) {
            swap(heapArray, parentIndex, smallestIndex);
